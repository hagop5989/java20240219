package ch06.lecture.p5static;

public class C01Static {
    public static void main(String[] args) {
        C01MyClass o1 = new C01MyClass();
        o1.name = "son";
        C01MyClass o2 = new C01MyClass();
        o2.name = "lee";

        // 참조변수로 static 멤버(필드, 메소드) 사용 권장하지 않음.
        C01MyClass.location = "seoul"; // <- 권장
//        o1.location = "seoul"; // <- 권장안함

        System.out.println("o1.name = " + o1);
        System.out.println("o2.name = " + o2);
        System.out.println("o1.location = " + C01MyClass.location);
        System.out.println("o2.location = " + C01MyClass.location);
    }
}

class C01MyClass {
    // 인스턴스 필드(인스턴스 변수) - 각 인스턴스가 가지고 있음
    String name;

    // static 필드(class 변수) - 인스턴스 필드x
    // 인스턴스가 아닌 클래스에 대한 정보(모든 객체가 공유함)
    static String location;

    // 인스턴스 메소드 - 인스턴스가 가지고 있는 기능
    void method1() {
    }
    // static 메소드는 클래스로 접근해서 실행시키는 것을 권장
//    o1.method2(); - xx
//    C01MyClass.method2(); - 권장
}